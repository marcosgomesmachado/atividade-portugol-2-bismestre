...
/*	Desafio: Sistema de Cadastro e Estatísticas
	...
	Idade
	Nota (0 a 10)
	Guarde os dados em vetores.

	Depois do cadastro, o programa deve mostrar:
	...
	Quantos alunos têm idade maior que a média das idades.
	Listar apenas os alunos aprovados.
	
	Regras

	Você não pode fazer tudo na função inicio().

	Crie pelo menos estas funções:

	real calcularMediaNotas(...)
	real calcularMediaIdades(...)
	inteiro contarAprovados(...)
	real maiorNota(...)
	real menorNota(...)
	cadeia alunoMaiorNota(...)
	cadeia alunoMenorNota(...)
	vazio listarAprovados(...)
	
	Desafio extra

	Depois de mostrar os resultados, exiba um menu:

	1 - Consultar aluno pelo nome
	2 - Alterar nota de um aluno
	3 - Mostrar estatísticas novamente
	4 - Sair

	O menu deve continuar aparecendo até o usuário escolher 4.

	Regras extras
	Não use variáveis globais.
	Passe os vetores como parâmetros para as funções.
	Evite repetir código.
	Faça validação da nota (aceite apenas valores entre 0 e 10). */
